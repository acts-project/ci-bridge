  ...
  test:
    ...
    steps:
      - run: echo "Triggered by GitLab"
        ...
        gh.getitem.assert_any_call(
            "/repos/myorg/myrepo/contents/.github/workflows/ci.yml"
        )
    ...
    @pytest.mark.asyncio
    async def test_trigger_github_workflow_no_compatible_workflows(
...
on:
  repository_dispatch:
    ...
    async def test_has_gitlab_workflow_detection(self, monkeypatch, config):
        ...
        workflows_response = {"workflows": [{"path": ".github/workflows/gitlab.yml"}]}
        gh.getitem.side_effect = [
            workflows_response,
            {
                "content": base64.b64encode(
...
jobs:
  test:
    ...
    @pytest.mark.asyncio
    async def test_on_job_hook_with_gitlab_to_github_triggering_enabled(
        self, app, monkeypatch, config
    ):
        ...
        monkeypatch.setattr(
            app.config, "GITLAB_TO_GITHUB_TRIGGER_ON_STATUS", ["success", "failed"]
        )
        ...
        get_pipeline_variables_mock = create_autospec(
            gitlab_client.get_pipeline_variables
        )
        ...
        get_client_for_installation_mock = create_autospec(
            github_utils.client_for_installation
        )
        get_client_for_installation_mock.return_value = mock_github_client
        monkeypatch.setattr(
            github_utils, "client_for_installation", get_client_for_installation_mock
        )

        handle_pipeline_status_mock = create_autospec(
            github_utils.handle_pipeline_status
        )
        monkeypatch.setattr(
            github_utils, "handle_pipeline_status", handle_pipeline_status_mock
        )

        # Mock GitLab to GitHub workflow triggering
        trigger_github_workflow_mock = create_autospec(
            github_utils.trigger_github_workflow
        )
        trigger_github_workflow_mock.return_value = True
        monkeypatch.setattr(
            github_utils, "trigger_github_workflow", trigger_github_workflow_mock
        )
        ...
        monkeypatch.setattr(
            "ci_relay.signature.Signature.verify", signature_verify_mock
        )
    ...
    async def test_on_job_hook_with_triggering_disabled(self, app, monkeypatch, config):
        ...
        # Mock GitLab client methods
        monkeypatch.setattr(
            gitlab_client, "get_pipeline", AsyncMock(return_value=mock_pipeline)
        )
        monkeypatch.setattr(
            gitlab_client,
        ...
        monkeypatch.setattr(
            gitlab_client, "get_project", AsyncMock(return_value=mock_project)
        )
        ...
        # Mock signature verification
        monkeypatch.setattr(
            "ci_relay.signature.Signature.verify", lambda self, payload, signature: True
        )
    ...
    @pytest.mark.asyncio
    async def test_on_job_hook_with_non_triggering_status(
        self, app, monkeypatch, config
    ):
        ...
        monkeypatch.setattr(app.config, "ENABLE_GITLAB_TO_GITHUB_TRIGGERING", True)
        monkeypatch.setattr(
            app.config, "GITLAB_TO_GITHUB_TRIGGER_ON_STATUS", ["success"]
        )
    ...
    @pytest.mark.asyncio
    async def test_trigger_github_workflow_error_handling(
        self, app, monkeypatch, config
    ):
    ...
    @pytest.mark.asyncio
    async def test_on_job_hook_feature_disabled_comprehensive(
        self, app, monkeypatch, config
    ):
